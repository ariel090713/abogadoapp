    ...
    public function run(): void
    {
        ...
        for ($i = 1; $i <= 20; $i++) {
            ...
            $lawyer = LawyerProfile::create([
                ...
                'graduation_year' => $faker->numberBetween(1990, 2020),
                'rating' => $faker->randomFloat(2, 3.5, 5.0),
                'total_reviews' => $faker->numberBetween(5, 100),
                'total_consultations' => $faker->numberBetween(10, 500),
                'is_verified' => true,
                'is_available' => true,
                'username' => strtolower(str_replace(' ', '', $user->name)) . $i,
                
                // Service offerings - randomly enable services
                'offers_chat_consultation' => $faker->boolean(80), // 80% chance
                'offers_video_consultation' => $faker->boolean(70), // 70% chance
                'offers_document_review' => $faker->boolean(60), // 60% chance
                
                // Chat consultation rates
                'chat_rate_15min' => $faker->randomElement([300, 400, 500, 600, 750]),
                'chat_rate_30min' => $faker->randomElement([500, 600, 750, 900, 1000]),
                'chat_rate_60min' => $faker->randomElement([900, 1000, 1200, 1500, 1800]),
                
                // Video consultation rates (typically higher than chat)
                'video_rate_15min' => $faker->randomElement([500, 600, 750, 900, 1000]),
                'video_rate_30min' => $faker->randomElement([900, 1000, 1200, 1500, 1800]),
                'video_rate_60min' => $faker->randomElement([1500, 1800, 2000, 2500, 3000]),
                
                // Document review minimum price
                'document_review_min_price' => $faker->randomElement([1000, 1500, 2000, 2500, 3000, 3500]),
            ]);
