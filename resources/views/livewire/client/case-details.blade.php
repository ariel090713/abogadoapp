...
<div class="p-8">
    ...
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('client.cases') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-6 mb-6">
        ...
        @if($case->description)
            <div class="border-t border-gray-200 pt-6 mt-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-3">Case Description</h3>
                <p class="text-gray-700 leading-relaxed whitespace-pre-wrap">{{ $case->description }}</p>
            </div>
        @endif

        <!-- Client's Legal Concern -->
        @if($case->client_notes)
            <div class="border-t border-gray-200 pt-6 mt-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-3">Your Legal Concern</h3>
                <p class="text-gray-700 leading-relaxed whitespace-pre-wrap">{{ $case->client_notes }}</p>
            </div>
        @endif
    </div>

    <!-- Pending Service Offers from Lawyer -->
    @php
        $pendingOffers = $case->childConsultations->where('status', 'pending_client_acceptance')->where('initiated_by', 'lawyer');
    @endphp
    
    @if($pendingOffers->count() > 0)
        <div class="bg-gradient-to-r from-orange-50 to-yellow-50 rounded-2xl shadow-lg border-l-4 border-orange-500 p-6 mb-6">
            <div class="flex items-start gap-4 mb-4">
                ...
                <div class="flex-1">
                    <h3 class="text-xl font-bold text-gray-900 mb-1">Service Offer{{ $pendingOffers->count() > 1 ? 's' : '' }} from Your Lawyer ({{ $pendingOffers->count() }})</h3>
                    <p class="text-gray-700">{{ $case->lawyer->name }} has offered additional service{{ $pendingOffers->count() > 1 ? 's' : '' }} for your case. Please review and respond.</p>
                </div>
            </div>

            <div class="space-y-3">
                @foreach($pendingOffers as $offer)
                    <div class="bg-white rounded-xl p-4 border border-orange-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-wrap items-center gap-2 mb-2">
                                <span class="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-semibold uppercase">
                                    {{ ucfirst(str_replace('_', ' ', $offer->consultation_type)) }}
                                </span>
                                @if($offer->quoted_price == 0)
                                    <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-semibold uppercase">
                                        FREE
                                    </span>
                                @else
                                    <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold">
                                        ₱{{ number_format($offer->quoted_price, 2) }}
                                    </span>
                                @endif
                            </div>
                            <h4 class="text-base font-bold text-gray-900 mb-1 break-words">{{ $offer->title }}</h4>
                            <p class="text-sm text-gray-600">
                                Offered {{ $offer->quote_provided_at->diffForHumans() }}
                            </p>
                        </div>
                        <div class="w-full sm:w-auto flex-shrink-0">
                            <a href="{{ route('client.consultation.details', $offer->id) }}" 
                               class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition font-medium text-sm w-full sm:w-auto">
                                View Offer
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                ...
                <!-- Request Additional Service Section -->
                <div class="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200 p-4 mb-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex-1">
                            <h3 class="text-base font-bold text-gray-900 mb-1">Need Additional Service?</h3>
                            <p class="text-sm text-gray-600">Request another consultation session, document review, or additional legal service for this case.</p>
                        </div>
                        <div class="w-full sm:w-auto flex-shrink-0">
                            @livewire('request-follow-up', ['consultation' => $case], key('request-follow-up-'.$case->id))
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    @foreach($sessions as $index => $session)
                        <a href="{{ route('client.consultation.details', $session->id) }}" class="block border border-gray-200 rounded-xl p-4 hover:border-primary-300 hover:shadow-xl transition cursor-pointer">
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <h3 class="text-base lg:text-lg font-semibold text-gray-900 mb-1 break-words">{{ $session->title }}</h3>
                        ...
                        <div class="flex gap-3">
                            ...
                            <div class="flex-1 pb-4">
                                ...
                                @elseif($event['type'] === 'session_completed')
                                    ...
                                    <p class="text-xs text-gray-600">{{ $event['data']->title }}</p>
                                    <p class="text-xs text-gray-500">with {{ $event['data']->lawyer->name }} (Lawyer)</p>
