...
<div class="p-4 sm:p-6 lg:p-8">
        ...
        <div class="flex items-start gap-3 sm:gap-4">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-sm text-gray-700 leading-relaxed">
                    Grouped consultations with the same lawyer - a thread can contain multiple related sessions (initial + follow-ups).
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        @forelse($cases as $case)
            ...
            <a href="{{ route('client.consultation-thread.details', $case->id) }}" 
               class="bg-white rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl hover:border-primary-300 transition flex flex-col cursor-pointer">
                <div class="p-6 flex-1">
                    ...
                    <div class="flex items-center gap-3 mb-4 pb-4 border-b border-gray-100">
                        @if($case->lawyer->lawyerProfile->profile_photo_url)
                            <img src="{{ $case->lawyer->lawyerProfile->profile_photo_url }}" 
                                 alt="{{ $case->lawyer->name }}"
                                 class="w-10 h-10 rounded-full object-cover flex-shrink-0">
                        @else
                            <div class="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center flex-shrink-0">
                                <span class="text-primary-600 font-semibold text-sm">{{ substr($case->lawyer->name, 0, 1) }}</span>
                            </div>
                        @endif
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-medium text-gray-900 truncate">{{ $case->lawyer->name }}</p>
        ...
        @empty
            <div class="bg-white rounded-2xl shadow-lg p-12 text-center">
                ...
                <p class="text-gray-600 mb-6">You don't have any consultation sessions yet. Start by booking a consultation with a lawyer.</p>
                <a href="{{ route('lawyers.search') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-primary-700 text-white rounded-lg hover:bg-[#1E40AF] transition">
                    Find a Lawyer
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                    </svg>
